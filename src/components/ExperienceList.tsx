    ...
    <div className="flex flex-col gap-1.5 md:gap-3">
      {experiences.map((exp, index) => (
        <div key={index} className="group">
          <h3 className="text-[8px] md:text-[10px] text-charcoal leading-tight md:leading-relaxed">
            {exp.role}
          </h3>
          <a
            href={exp.companyUrl}
            ...
            className="text-[7px] md:text-[9px] text-muted hover:text-coral transition-colors"
          >
            {exp.company}
          </a>
          <p className="text-[6px] md:text-[8px] text-muted/70 mt-0.5">{exp.period}</p>
        </div>
      ))}
    </div>
  );
